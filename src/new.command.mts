...
async function run(args: argsValid) {
  ...
  const packageJsonContent: any = packageJson
...
const createPackageJson = (folderName: string, packageJsonContent: any) => {
...
async function installPackages(folderName: string, packages: string[]) {
  const command = `cd ${folderName} && npm install ${packages.join(' ')}`;
  const {stdout, stderr} = await promiseExec(command).catch((error: Error) => {
    if (error) {
